...
// 조건부 렌더링 :
// ? 상황 (조건)에 따라서 렌더링 결과를 다르게 렌더링 하려할 때 사용하는 방법
// * if, &&, ? : 을 사용하는 방법이 존재함
...
function IfComponent({ number }: { number: number }) {
  ...
  if (number > 0) return <h1 style={{ color: "red" }}>{number}</h1>;
  // ? 음수이면 number 값을 h3이면서 파란색으로 표시
  else if (number < 0) return <h3 style={{ color: "blue" }}>{number}</h3>;
...
function ThreeTermComponent({ number }: { number: number }) {
  ...
  // JSX 내부에서(return 안) 상황(조건)에 따라서 서로 다른 렌더링을 하고 싶을 떄 사용

  // ex) number가 양수면 '양수', 음수면 '음수' ,0이면 '영'

  return <h1>{number > 0 ? "양수" : number < 0 ? "음수" : "영"}</h1>;
}
