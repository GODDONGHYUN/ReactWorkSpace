...
export default function StateComponent() {
  // status 선언:
  // * useState 함수로 함수를 선언
  // ^ const [상태변수, 상태변경 함수] = useState<상태변수타입>(초기값);
  ...
  const onCountAddHandler = () => {
    // setCount(count + 1);
    // ! 상태 변수는 반드시 상태변경함수로 변경해야 리렌더링 됨 !!!!!
    // ? 상태변경 함수를 통해서 함수를 변경한다고 바로 적용 되지 않음
  ...
  const onAddListHandler = () => {
    // ^ 상태가 배열 혹은 객체이면 각각에 대해 요소를 추가, 변경이 일어나도 상태가 변경된 것으로 인식 하지 않음(실제 주소 변경 X)
    // ^ 상태가 변경된 것도 인식하게 하고 싶으면 새로운 배열 혹은 객체를 생성하여 상태를 변경해야함(일반적으로 복사해서 변경)
  ...
  return (
    <>
      ...
      <h1>{total}</h1>
      <hr />
      {/* Input이 만약 상태를 변경한다면 value로 그 상태를 지정해야 불일치가 발생
      하지 않음 */}
