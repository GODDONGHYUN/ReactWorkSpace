...
export default function EventComponent() {
  ...
  // ^ onClick : 클릭이 발생 했을 때
  // ^ onChange : input 요소에서 값이 변경 됐을 때
  // ^ onKeyDown : input 요소에서 키보드 입룍이 발생 했을 때

  // ? 각 이벤트의 콜백 함수는 이벤트 객체를 매개변수로 받을 수 있음
  // ? TypeScript는 이벤트 객체의 타입은 "특정 Event<이벤트가 발생할 요소>"
  const onKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key !== "Enter") return onClickHandler();
  };
