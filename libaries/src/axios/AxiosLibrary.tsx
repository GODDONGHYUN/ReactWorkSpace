...
export default function AxiosLibrary() {
  ...
  axios
    .get(GET_URL)
    //  각각의 http method 함수 결과는 then으로 받을 땐 response 객체를 매개변수로 받는 콜백 함수를 전달
    .then((response) => {
    ...
    .catch((error) => {
      // ? 해당 request에 대해서 request가 실패하거나 status 코드가 400, 500번대 이면 catch로 AxiosError 객체를 받아옴
      // ? response가 존재하는 에러는 error 객체에 response가 포함되어 있음
      console.log(error);
    });

  // # option 매개변수
  // ? request의 각종 설정을 조작할 때 사용 됨
  axios.get(GET_URL, {
  ...
  // # request body 매개변수
  // ? post, patch, put 함수는 두 번째 매개변수로 request body 데이터를 전달함
